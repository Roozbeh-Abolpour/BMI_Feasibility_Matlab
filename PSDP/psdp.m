function [x,y,T]=psdp(Ls,Bs,bounds,params)
lbx=bounds.lbx;ubx=bounds.ubx;
lby=bounds.lby;uby=bounds.uby;
nx=length(lbx);ny=length(lby);
conv_ther=params.conv_ther;
accept_ther=params.accept_ther;
penalty_gain=params.penalty_gain;
tmax=params.tmax;
T=0;
tic
[~,Bss]=extendedform(Ls,Bs,nx,ny);
T=T+toc;
xb=0.5*(lbx+ubx);yb=0.5*(lby+uby);
while T<=tmax
    tic
    yalmip('clear');
    x=sdpvar(nx,1);y=sdpvar(ny,1);
    Z=sdpvar(nx+ny,nx+ny,'symmetric');
    g=sdpvar(1,1);
    F=[x>=lbx,x<=ubx,y>=lby,y<=uby];
    for i=1:length(Ls)
        L=Ls{i}(x,y);
        F=[F,L<=g*eye(length(L))];
    end
    for i=1:length(Bss)
        B=Bss{i}{1};
        for j=1:nx
            B=B+Bss{i}{1+j}*x(j);
        end
        for j=1:ny
            B=B+Bss{i}{1+nx+j}*y(j);
        end
        for j=1:nx
            for k=1:ny
                B=B+Bss{i}{1+nx+ny+(j-1)*ny+k}*Z(j,nx+k);
            end
        end
        F=[F,B<=g*eye(length(B))];
    end
    z=[x;y];
    F=[F,[-1 z';z -Z]<=0];
    gc=penalty_gain*(trace(Z)-2*xb'*x-2*yb'*y);    
    ops=sdpsettings;ops.solver='mosek';ops.verbose=0;
    sol=optimize(F,g+gc,ops);
    if sol.problem==0
        x=value(x);y=value(y);
        g=value(g);
    else
        x=xb;y=yb;
        T=T+toc;
        return
    end
    if norm(x-xb)+norm(y-yb)<=conv_ther
        T=T+toc;
        return
    end
    if g<=accept_ther
        T=T+toc;
        return
    end
    T=T+toc;
    xb=x;yb=y;
end
end
